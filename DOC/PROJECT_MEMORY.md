...
> Private exercise media workflow (2026-09-24): a trainer can persist an exercise title, instruction, ordered groups of muscles and one private S3 instructional video. Videos accept MP4, WebM or MOV up to 200 MiB through a direct presigned PUT, remain private, and become attached only after confirmation. Every new cover, whether selected from a video or uploaded as JPEG/PNG/WebP, is private, centrally cropped to 16:9, normalized by `thumbnail-worker` into 320×180 WebP no larger than 100 KiB, and attached only after it is ready. Trainer and client lists obtain short-lived cover URLs in batches; client authorization remains bound to the immutable workout snapshot. Legacy URL-based and older private covers remain compatible.
