...
> Private exercise media workflow (2026-09-23): a trainer can now persist an exercise title, instruction, ordered groups of muscles and one private S3 instructional video. Videos accept MP4, WebM or MOV up to 200 MiB through a direct presigned PUT, remain private, and become attached only after confirmation. Each video receives a private JPEG cover automatically from a frame around the first second; the trainer can replace it with a paused video frame or a JPEG/PNG/WebP image up to 5 MiB. Workout assignments snapshot both media IDs with the exercise, so a client receives short-lived playback and cover URLs only when their active relationship and immutable assignment snapshot authorize those exact objects; legacy URL-based exercise data remains compatible.

> Trainer navigation consolidation (2026-09-23): the trainer desktop sidebar is now owned by one standalone `TrainerSidebarComponent` with a typed route and SVG-icon model. Trainer screens must consume this component rather than duplicate sidebar markup, icons or active-state rules; mobile navigation remains independently scoped.
...
> Program tasks and private media (2026-09-22): `tasks` and `media` modules are implemented in an isolated `codex/program-tasks-s3` worktree, not deployed. Program schedules accept ordered `WORKOUT` and `TASK` items; issuing a program materializes immutable task assignments, while cancelling a program cancels pending task children. Task results are append-only versions and may contain validated measurements, completion flags, notes and confirmed private-image IDs. S3 uploads use signed PUT URLs, server confirmation and short-lived signed read URLs; a trainer obtains a photo URL only via an authorized task-result route. The trainer program builder can choose a task template as a scheduled program item. Migrations 20260922_0011 and 20260922_0012, OpenAPI and generated TypeScript contracts are prepared; application of migrations and deployment remain pending owner approval.
