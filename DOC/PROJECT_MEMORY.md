...
> Private exercise media workflow (2026-09-23): a trainer can now persist an exercise title, instruction, ordered groups of muscles and one private S3 instructional video. Videos accept MP4, WebM or MOV up to 200 MiB through a direct presigned PUT, remain private, and become attached only after confirmation. Workout assignments snapshot the media ID with the exercise, so a client receives a short-lived playback URL only when their active relationship and immutable assignment snapshot authorize that exact video; legacy URL-based exercise data remains compatible.
