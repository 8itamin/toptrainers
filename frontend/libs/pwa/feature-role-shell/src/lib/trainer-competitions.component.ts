...
@Component({
  ...
  standalone: true,
  imports: [NgTemplateOutlet, RouterLink, TrainerSidebarComponent],
  template: `
    ...
    <div class="competitions-shell">
      ...
      <nav class="mobile-nav mobile-only" aria-label="Навигация тренера">@for (item of mobileNavItems; track item.path) { <a [class.is-active]="item.path === '/trainer/competitions'" [routerLink]="item.path"><span><ng-container *ngTemplateOutlet="navIcon; context: { $implicit: item.icon }" /></span>{{ item.label }}</a> }</nav>
    </div>
  `,
  styles: `
    ...
    @media (min-width:860px) and (max-width:1079.98px){.rules{display:none}.competitions-shell{display:block}.main{min-height:100dvh;padding-bottom:6.5rem}.mobile-only{display:flex}.mobile-nav{position:fixed;z-index:10;inset-inline:0;bottom:0;justify-content:space-around;padding:.625rem 1rem calc(.625rem + env(safe-area-inset-bottom));background:rgb(14 17 22 / 96%);border-top:1px solid rgb(245 247 250 / 8%);backdrop-filter:blur(12px)}.mobile-nav a{display:flex;flex-direction:column;align-items:center;gap:.2rem;color:#8a94a6;text-decoration:none;font-size:.625rem}.mobile-nav a span{font-size:1.125rem;line-height:1}.mobile-nav .is-active{color:#c9f24b;font-weight:700}}
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TrainerCompetitionsComponent {
  protected readonly mobileNavItems = TRAINER_NAVIGATION.filter((item) => item.id !== 'showcase').map((item) =>
    item.id === 'competitions' ? { ...item, label: 'Ещё' } : item,
  );
