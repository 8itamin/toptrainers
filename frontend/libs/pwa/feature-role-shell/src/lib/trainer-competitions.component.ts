...
import { TRAINER_PROGRAM_BUILDER_NAVIGATION } from './program-builder-state';
...
@Component({
  ...
  template: `
    ...
    <div class="competitions-shell">
      <aside class="sidebar desktop-only">
        ...
        <nav class="sidebar-nav" aria-label="Навигация тренера">
          @for (item of navItems; track item.path) {
            <a class="side-item" [class.is-active]="item.path === '/trainer/competitions'" [routerLink]="item.path"><span class="side-icon"><ng-container *ngTemplateOutlet="navIcon; context: { $implicit: item.icon }" /></span><span>{{ item.label }}</span></a>
          }
      ...
      <nav class="mobile-nav mobile-only" aria-label="Навигация тренера">@for (item of navItems; track item.path) { <a [class.is-active]="item.path === '/trainer/competitions'" [routerLink]="item.path"><span><ng-container *ngTemplateOutlet="navIcon; context: { $implicit: item.icon }" /></span>{{ item.label }}</a> }</nav>
    </div>
  `,
  styles: `
...
export class TrainerCompetitionsComponent {
  protected readonly navItems = TRAINER_PROGRAM_BUILDER_NAVIGATION.map((item) =>
    item.path === '/trainer/competitions' ? { ...item, label: 'Соревн.' } : item,
  );
