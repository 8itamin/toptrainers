...
@Component({
  ...
  template: `
    ...
    <section class="card"><h2>Доступ тренера</h2><p>Создайте первый аккаунт тренера или войдите в существующий.</p>
      <form class="program-form" (ngSubmit)="authenticate()"><label>Email<input name="email" type="email" [(ngModel)]="email" required autocomplete="email" /></label><label>Пароль<input name="password" type="password" [(ngModel)]="password" required minlength="12" autocomplete="current-password" /></label><div class="auth-actions"><button class="button button--primary" type="submit" [disabled]="authBusy()">{{ authBusy() ? 'Проверяем…' : 'Войти' }}</button><button class="button button--secondary" type="button" [disabled]="authBusy()" (click)="authenticate(true)">Зарегистрироваться</button></div></form>
      @if (authMessage()) { <p class="form-message">{{ authMessage() }}</p> }
    </section>
...
export class TrainerHomeComponent {
  ...
  protected authenticate(register = false): void {
    this.authBusy.set(true);
    this.authMessage.set('');
    const path = register ? 'register' : 'login';
    const body = register
      ? { email: this.email, password: this.password, role: 'trainer' }
      : { email: this.email, password: this.password };
    this.http.post<{ access_token: string }>(`${this.config.apiBaseUrl}/auth/${path}`, body).subscribe({
      next: ({ access_token }) => { localStorage.setItem('tt_access_token', access_token); this.authMessage.set(register ? 'Аккаунт тренера создан' : 'Вы вошли'); },
      error: () => { this.authMessage.set(register ? 'Не удалось создать аккаунт. Проверьте email и пароль от 12 символов.' : 'Не удалось войти. Проверьте данные.'); this.authBusy.set(false); },
      complete: () => this.authBusy.set(false),
    });
  }
  ...
  protected createProgram(): void {
    ...
    this.http.post<Program>(`${this.config.apiBaseUrl}/programs`, { title: this.title, description: this.description, weeks: this.weeks }, { headers }).subscribe({
      ...
      error: () => { this.message.set('Нужна авторизация тренера. Следующим шагом добавим форму входа.'); this.busy.set(false); },
